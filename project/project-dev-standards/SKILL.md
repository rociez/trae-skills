...
| `/output/数据模型设计.md` | 理解实体结构（影响类型定义规范） |
| `/output/` 下所有已有文档 | 全量扫描，确保不遗漏任何已定义的约束 |
...
| **与目录结构一致** | 文件命名规则、模块划分是否与 `/output/项目目录结构.md` 一致 |
| **与 API 规范一致** | 错误处理、认证检查规则是否与 `/output/API设计规范.md` 一致 |
| **无自相矛盾** | 各章节之间的规则是否存在冲突 |
| **示例代码可运行** | 示例代码是否语法正确、可直接复制使用 |

### 第五步：生成文档

1. 读取 [`dev-standards-template.md`](https://github.com/rociez/trae-skills/blob/main/template/dev-standards-template.md) 作为生成基准。
2. 按模板结构填充所有章节内容。
3. 保存为 `/output/开发规范.md`。
...
- **最终交付**：当文档内容被用户确认后，请将其保存到 `/output/开发规范.md`。
